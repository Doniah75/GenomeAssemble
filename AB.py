...
def generate_plots(results):
    ...
    # Heatmap for min_overlap vs N (Error Rate)
    min_overlap_values = sorted(list(set(x[1] for x in results if x[0] == 'min_overlap')))
    N_values = sorted(list(set(x[1] for x in results if x[0] == 'N')))
    error_rate_grid = np.zeros((len(min_overlap_values), len(N_values)))

    # Populate the error rate grid
    for result in results:
        if result[0] == 'min_overlap':
            min_overlap_index = min_overlap_values.index(result[1])
            for N_index, N_val in enumerate(N_values):
                # Find the corresponding result for this N and min_overlap
                corresponding_result = next((x for x in results if x[0] == 'N' and x[1] == N_val and x[2][6] is not None), None)
                if corresponding_result:
                    error_rate_grid[min_overlap_index, N_index] = corresponding_result[2][6]  # Error-prone error rate

    # Plot the heatmap
    plt.figure(figsize=(10, 8))
    sns.heatmap(error_rate_grid, annot=True, fmt=".4f", xticklabels=N_values, yticklabels=min_overlap_values, cmap="YlOrRd", cbar_kws={'label': 'Error Rate'})
    ...
    plt.savefig("plots/error_rate_heatmap_min_overlap_vs_N.png", bbox_inches='tight', transparent=True)
    plt.close()

    # 3D Plot for N, l, and error rate
    # Filter results to include only experiments where both N and l are varied
    filtered_results = [x for x in results if x[0] in ['N', 'l']]
    N_values = [x[1] for x in filtered_results if x[0] == 'N']
    l_values = [x[1] for x in filtered_results if x[0] == 'l']
    error_rate_values = [x[2][6] for x in filtered_results if x[0] == 'N']  # Error-prone error rate

    # Ensure all arrays have the same length
    min_length = min(len(N_values), len(l_values), len(error_rate_values))
    N_values = N_values[:min_length]
    l_values = l_values[:min_length]
    error_rate_values = error_rate_values[:min_length]

    # Create the 3D plot
    fig = plt.figure(figsize=(12, 8))
    ax = fig.add_subplot(111, projection='3d')

    # Use a darker colormap for better visibility
    colors = error_rate_values
    scatter = ax.scatter(N_values, l_values, error_rate_values, c=colors, cmap='inferno', marker='o', s=100, depthshade=True)

    # Add text labels for each point
    for i in range(len(N_values)):
        ax.text(N_values[i], l_values[i], error_rate_values[i], f'{error_rate_values[i]:.4f}', color='white', fontsize=10)
    ...
    plt.savefig("plots/3d_plot_N_l_error_rate.png", bbox_inches='tight', transparent=True)
    plt.close()
